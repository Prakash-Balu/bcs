    ...
    if($status==3)
        {
         ...
         if( empty($_GET["sid"])) {
          ...
          $quotation = $_GET["sid"];
                ...
                $sqlSubject="SELECT * FROM `tbl_quatation` WHERE `qt_qtno`='$quotation'";
                //print $sqlSubject; 
                ...
                if($rowcount>0) {
                    ...
                    for($i=1;$i<=$rowcount;$i++)
                    {
                        ...
                        $qt_date = $res['qt_qtdate'];
                        $qt_no = $res['qt_qtno'];
                        $qt_expdate = $res['qt_expdate'];
                        $qt_buydet = $res['qt_buydet'];
                        $qt_other = empty($res['iv_sgst']) ? 'yes':'no';
                        $qt_prdtl[] = array("p_name"=>$res['qt_prdname'], "p_price"=>$res['qt_prdprice'], "p_qty"=>$res['qt_prdqty'],"p_amt"=>$res['qt_prdamount']);
                        $total_amt = $total_amt + $res['qt_prdamount'];
                    }
                }
        }
        ?>
        <script type="text/javascript">
            ...
            window.addEventListener('DOMContentLoaded', (event) => {
                ...
                console.log(rowCnt);
                if (rowCnt > 0) {
                    document.getElementById("txt_qtno").value = "<?php echo $qt_no; ?>";
                    document.getElementById("date1").value = "<?php echo $qt_date; ?>";
                    document.getElementById("buyer_Address").value = <?php echo json_encode($qt_buydet); ?>;
                    var otherState = "<?php echo $qt_other; ?>";
                    document.getElementById("date2").value = "<?php echo $qt_expdate; ?>";
                    ...
                    var prdDtl = <?php echo json_encode($qt_prdtl); ?>;
                    console.log(prdDtl);
                    if (prdDtl) {
                        var j = 1;
